...
export default function App() {
  ...
  useEffect(() => {
    ...
    (window as any).openDetailedGapOfferPopup = (id: string) => {
      const found = gapsOffers.find(g => g.id === id);
      if (found) {
        setDetailedGap(found);
      }
    };
    ...
    return () => {
      ...
      delete (window as any).openDetailedGapOfferPopup;
      delete (window as any).openContactPopup;
    };
  }, [organizations, gapsOffers]);
  ...
  useEffect(() => {
    ...
    filteredOrganizations.forEach((org) => {
      ...
      // Popup layout
      const popupHtml = `
        <div class="p-3.5 font-sans w-52 space-y-2 text-left">
          <div>
            <h3 class="text-xs font-bold text-[#1a2521] leading-snug break-words m-0">${org.name}</h3>
            <p class="text-[10px] text-[#51615a] m-0 mt-1 flex items-center gap-1.5">
              <span class="inline-block w-2 h-2 rounded-full shrink-0" style="background-color: ${markerColor}"></span>
              <span class="truncate font-medium">${org.sector} | ${org.location}</span>
            </p>
          </div>
          <div class="pt-0.5 flex flex-col gap-1.5">
            <button onclick="window.openDetailedOrgPopup('${org.id}')" class="w-full text-center py-1.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-[10px] text-white font-bold rounded-md transition cursor-pointer shadow-2xs">
              View details
            </button>
            <button onclick="window.openContactPopup('${org.id}')" class="w-full text-center py-1 bg-white hover:bg-[#fbfbf9] border border-[#e1e1db] text-[9px] text-[#51615a] font-medium rounded-md transition cursor-pointer">
              Send Message
            </button>
          </div>
        </div>
...
`;
  ...
  const activeTabClass = (tab: AlphaTab) => {
    return currentAlphaTab === tab 
      ? 'border-[#29B6BD] text-[#176e73] bg-[#29B6BD]/10 font-bold shadow-2xs' 
      : 'border-transparent text-[#51615a] hover:text-[#1a2521] hover:bg-teal-50/50';
  };
              ...
              <div className="border-t border-[#e1e1db] pt-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3 bg-slate-50 rounded-xl border border-gray-100 flex items-start gap-3">
                    <div className="p-1 bg-[#29B6BD]/10 rounded-lg text-[#176e73] mt-0.5">
              ...
              <div className="bg-white rounded-2xl border border-[#e1e1db] overflow-hidden shadow-xs">
                <div className="bg-[#1a2521] px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-white">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
                  >
  ...
  CREATE POLICY "Allow public insert on learning" ON wales_evidence_learning FOR INSERT WITH CHECK (true);`}
        ...
        {/* 2. JOURNEY & PATHWAY DIAGNOSTIC VIEW (INTEGRATED WITH STAGE HEATMAP & GAPS/OFFERS) */}
        {(currentAlphaTab === 'journey' || currentAlphaTab === 'gaps_offers') && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <GapsHeatmap 
              ...
              gapCategoryFilter={gapCategoryFilter}
              onGapCategoryFilterChange={setGapCategoryFilter}
            ...
            <div className="bg-white rounded-2xl border border-[#e1e1db] p-5 shadow-xs">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
                <div className="space-y-1.5 max-w-sm">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#29B6BD] animate-pulse" />
                    <h3 className="text-[10px] font-extrabold uppercase tracking-widest text-[#176e73]">
                ...
                <button 
                  ...
                  className="px-4 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-xs font-bold text-white rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs self-start lg:self-center shrink-0"
                >
              ...
              {/* Submissions & Initiatives Ledger Section */}
              <div className="bg-white rounded-2xl border border-[#e1e1db] overflow-hidden shadow-xs">
                {/* Collapsible Header Bar */}
                <div 
                  onClick={() => setShowAllSubmissions(!showAllSubmissions)}
                  className="px-6 py-4 bg-[#F4F4F0] border-b border-[#e1e1db] flex flex-wrap items-center justify-between gap-4 cursor-pointer hover:bg-[#e1e1db]/30 transition"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-[#29B6BD] text-white flex items-center justify-center font-bold text-xs shadow-2xs">
                      {filteredGapsOffers.length}
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-[#1a2521] leading-tight">
                        Stage Initiatives, Gaps & Offers Ledger
                      </h3>
                      <p className="text-[11px] text-[#51615a]">
                        {showAllSubmissions 
                          ? `Displaying all ${filteredGapsOffers.length} logged entries on map.` 
                          : `Showing top 3 featured entries. Click to ${showAllSubmissions ? 'collapse' : `expand all ${filteredGapsOffers.length} entries`}.`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {showAllSubmissions && (
                      <div className="flex items-center bg-white border border-[#e1e1db] rounded-xl p-1 text-[11px]">
                        <button
                          onClick={() => setSubmissionsViewMode('grid')}
                          className={`px-2.5 py-1 rounded-lg font-bold transition cursor-pointer ${
                            submissionsViewMode === 'grid' ? 'bg-[#29B6BD] text-white' : 'text-[#51615a] hover:text-[#1a2521]'
                          }`}
                        >
                          Grid View
                        </button>
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg font-bold transition cursor-pointer ${
                            submissionsViewMode === 'table' ? 'bg-[#29B6BD] text-white' : 'text-[#51615a] hover:text-[#1a2521]'
                          }`}
                        >
                          Compact Table
                        </button>
                    ...
                    <button
                      onClick={() => setShowAllSubmissions(!showAllSubmissions)}
                      className="px-3.5 py-1.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white rounded-xl text-xs font-bold transition cursor-pointer flex items-center gap-1.5 shadow-2xs"
                    >
                      <span>{showAllSubmissions ? 'Collapse List' : `View All (${filteredGapsOffers.length})`}</span>
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${showAllSubmissions ? 'rotate-180' : ''}`} />
                    </button>
                  </div>
                </div>
                ...
                <div className="p-6">
                  {loadingGaps ? (
                    <div className="py-12 text-center text-xs text-[#51615a]">Loading submissions...</div>
                    ...
                    <div className="bg-white border border-[#e1e1db] rounded-2xl p-8 text-center space-y-3">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setGapUrgencyFilter('All');
                          setSelectedRegionFilter('All');
                        }}
                        className="px-4 py-2 bg-[#29B6BD]/10 hover:bg-[#29B6BD]/20 text-[#176e73] rounded-xl text-xs font-bold transition cursor-pointer"
                      >
                        Reset All Filters
                      </button>
                    </div>
                  ) : showAllSubmissions && submissionsViewMode === 'table' ? (
                    /* COMPACT TABLE VIEW FOR CLEAN UNCLUTTERED DISPLAY */
                    <div className="overflow-x-auto rounded-xl border border-[#e1e1db]">
                      <table className="w-full text-left text-xs">
                        <thead className="bg-[#F4F4F0] border-b border-[#e1e1db] text-[#51615a] uppercase text-[9px] font-bold tracking-wider">
                          <tr>
                            <th className="p-3">Type / Category</th>
                            <th className="p-3">Title & Summary</th>
                            <th className="p-3">Organisation</th>
                            <th className="p-3">Region & Stage</th>
                        ...
                        <tbody className="divide-y divide-[#e1e1db]">
                          {filteredGapsOffers.map((item) => {
                            ...
                            if (item.type === 'Offer') badgeColor = 'bg-emerald-50 border-emerald-200 text-emerald-700';
                            else if (item.type === 'Request') badgeColor = 'bg-amber-50 border-amber-200 text-amber-700';
                            else if (item.type === 'Collaboration') badgeColor = 'bg-blue-50 border-blue-200 text-blue-700';
                              ...
                              <tr key={item.id} className="hover:bg-slate-50 transition">
                                ...
                                <td className="p-3 whitespace-nowrap text-right">
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-[#29B6BD]/10 hover:bg-[#29B6BD]/20 text-[#176e73] font-bold rounded-lg text-[10px] transition cursor-pointer"
                                  >
                                    Inspect
                                  </button>
                                </td>
                    ...
                    /* CARD GRID VIEW (Showing top 3 when collapsed, or all when expanded) */
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {(showAllSubmissions ? filteredGapsOffers : filteredGapsOffers.slice(0, 3)).map((item) => {
                          let badgeColor = 'bg-rose-50 border-rose-200 text-rose-700';
                          if (item.type === 'Offer') {
                          ...
                          return (
                            <div key={item.id} className="bg-white rounded-2xl border border-[#e1e1db] p-5 flex flex-col justify-between space-y-4 hover:shadow-xs transition duration-200">
                              <div className="space-y-3">
                                ...
                                <p className="text-[11px] text-[#51615a] leading-relaxed line-clamp-3">
                                  {item.content}
                                </p>
                              </div>
                              ...
                              <div className="pt-3 border-t border-[#e1e1db] flex items-center justify-between text-[10px] text-[#51615a]">
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={() => setDetailedGap(item)}
                                    className="text-[#29B6BD] hover:underline font-bold flex items-center gap-0.5 cursor-pointer"
                                  >
                      ...
                      {/* Expand / Collapse Button Bar below cards */}
                      {!showAllSubmissions && filteredGapsOffers.length > 3 && (
                        <div className="text-center pt-2">
                          <button
                            onClick={() => setShowAllSubmissions(true)}
                            className="px-5 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-xl text-xs transition cursor-pointer shadow-2xs inline-flex items-center gap-2"
                          >
                            <span>Show All {filteredGapsOffers.length} Logged Entries</span>
                ...
                <div className="bg-white border border-[#e1e1db] rounded-2xl p-4 space-y-3 shadow-xs">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-4 py-2 text-xs border border-[#e1e1db] rounded-xl focus:outline-none focus:border-[#29B6BD]"
                    />
                  </div>
                      ...
                      <div
                        ...
                        className={`p-4 rounded-2xl border text-left cursor-pointer transition-all duration-200 ${
                          org.id === selectedOrgId
                            ? 'bg-white border-[#29B6BD] shadow-sm ring-1 ring-[#29B6BD]/20'
                      ...
                      >
                        ...
                        <div className="mt-3 pt-2.5 border-t border-[#e1e1db] flex flex-wrap items-center justify-between gap-2 text-[10px] text-[#51615a]">
                          ...
                          <button
                            ...
                            className="text-[#29B6BD] hover:underline font-bold"
                          >
                            Read Profile
                          </button>
                        </div>
            ...
            <div className="bg-white rounded-2xl border border-[#e1e1db] p-5 shadow-xs flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#29B6BD] hover:bg-[#1d8e93] text-xs font-bold text-white rounded-xl flex items-center gap-1.5 shadow-xs transition cursor-pointer"
                >
                  <Plus className="w-4 h-4 text-white" />
                      ...
                      filteredCommitments.map((item) => {
                        ...
                        return (
                          <tr key={item.id} className="hover:bg-slate-50/50 transition">
                            ...
                            <td className="p-4">
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-[#29B6BD]/10 text-[#176e73]">
                                {item.assignedTab}
                              </span>
                            </td>
        ...
        {currentAlphaTab === 'evidence_learning' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            {/* Main Content Grid: 2/3 Log Feed on Left, 1/3 Project Showcase Carousel on Right */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              
              {/* Left 2/3 Column: Timeline Log Feed */}
                    ...
                    return (
                      <div key={log.id} className="bg-white border border-[#e1e1db] rounded-2xl p-5 shadow-xs flex gap-4 hover:border-gray-300 transition duration-150 text-left">
                        <div className="flex-shrink-0 flex flex-col items-center">
                          <div className="w-9 h-9 rounded-full bg-[#29B6BD]/10 flex items-center justify-center text-[#176e73]">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {log.whatChanged && (
                            <div className="p-3 bg-slate-50 border-l-2 border-[#29B6BD] rounded-r-xl text-[10px] text-[#51615a] leading-normal">
            ...
            <form onSubmit={handleAddInitiativeSubmit} className="space-y-4 pt-4 text-xs">
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="w-4 h-4 rounded text-[#29B6BD] focus:ring-[#29B6BD]"
                />
              ...
              <div className="pt-4 flex gap-3">
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-xl shadow-xs transition duration-150 cursor-pointer text-center"
                >
        ...
        <div className="fixed inset-0 bg-[#1a2521]/45 backdrop-blur-xs flex items-center justify-center p-4 z-[9999] overflow-y-auto">
          <div className="bg-white rounded-3xl border border-[#e1e1db] w-full max-w-lg shadow-2xl p-6 md:p-8 animate-scaleIn text-left">
            ...
            <form onSubmit={handleGapSubmit} className="space-y-4 pt-4 text-xs">
              ...
              <div className="pt-4 flex gap-3">
                <button type="submit" className="flex-1 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-xl cursor-pointer">
                  Save Entry
                </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="border border-[#e1e1db] rounded-2xl p-4 bg-white space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <span className="text-[9px] text-gray-400 block font-medium">Organisation</span>
                    ...
                    {detailedGap.contactEmail && (
                      <div>
                        ...
                        <span className="text-[11px] font-semibold text-[#176e73]">{detailedGap.contactEmail}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="pt-4 border-t border-[#e1e1db] flex gap-3">
              {detailedGap.contactEmail ? (
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-xl text-xs transition cursor-pointer text-center flex items-center justify-center gap-1.5"
                >
          ...
          <div className="bg-white rounded-3xl border border-[#e1e1db] w-full max-w-md shadow-2xl p-6 animate-scaleIn text-left">
            <div className="flex items-center justify-between pb-3 border-b border-[#e1e1db]">
              <div>
                ...
                <p className="text-[11px] text-[#51615a]">Relay a prompt to: <span className="font-bold text-[#176e73]">{contactOrg.name}</span></p>
              </div>
            ...
            <form onSubmit={handleContactSubmit} className="space-y-4 pt-3.5 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleContactChange}
                  className="w-full mt-1 px-3 py-2 border border-[#e1e1db] bg-white rounded-xl focus:outline-none focus:border-[#29B6BD]"
                >
              ...
              <div className="pt-3 flex gap-2">
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-xl transition duration-150 cursor-pointer text-center"
                >
                  Relay Prompt
                </button>
                <button
        ...
        <div className="fixed inset-0 bg-[#1a2521]/45 backdrop-blur-xs flex items-center justify-center p-4 z-[9999]">
          <div className="bg-white rounded-3xl border border-[#e1e1db] w-full max-w-2xl shadow-2xl p-6 animate-scaleIn text-left flex flex-col max-h-[92vh]">
            <div className="flex items-start justify-between pb-3.5 border-b border-[#e1e1db] flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-[#29B6BD]/10 rounded-2xl text-[#176e73] flex items-center justify-center">
              ...
              <div className="bg-slate-50 border border-[#e1e1db] p-4 rounded-2xl text-xs space-y-2">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] font-mono text-[#176e73]">{currentUser.contactEmail}</span>
                  </div>
                        ...
                        <div key={savedId} className="p-3 border border-gray-200 rounded-xl bg-white hover:bg-slate-50 transition flex items-center justify-between gap-3 text-xs">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-[#29B6BD] hover:bg-[#1d8e93] text-white font-bold rounded-lg transition text-[10px]"
                            >
                              Inspect
                            </button>
                            <button
