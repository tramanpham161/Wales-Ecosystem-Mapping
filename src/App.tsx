...
export default function App() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fbfbf9] font-sans antialiased text-[#1a2521]">
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-3">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-[#1a2521] tracking-tight">
              Wales Employment Ecosystem Map
            </h1>
          </div>
      ...
      {/* 3. Systemic Friction Points Tabs (Prominent 6-tab navigation) */}
      <nav className="flex-none bg-white border-b border-[#e1e1db] px-6 py-3 overflow-x-auto custom-scrollbar relative z-10">
        <div className="max-w-7xl mx-auto flex gap-3 min-w-[980px]">
          {SYSTEMIC_TABS.map((tab) => {
            const isActive = activeTab === tab.id;
            const activeColor = tabColorHex[tab.id];
            ...
            return (
              <button
                ...
                onClick={() => {
                  setActiveTab(tab.id);
                  setSelectedOrgId(null); // Clear active item so users browse freely
                }}
                className={`flex-1 flex items-center justify-center p-3 rounded-xl border text-center transition-all duration-200 relative group cursor-pointer ${
                  isActive
                    ? 'bg-white border-[#1a2521] shadow-[0_4px_12px_rgba(26,37,33,0.06)] font-bold'
                    : 'bg-[#fbfbf9] border-[#e1e1db] hover:border-[#51615a]/40 hover:bg-white font-medium'
                }`}
                style={{
                  borderLeft: isActive ? `4px solid ${activeColor}` : '1px solid #e1e1db'
                }}
              >
                <span className={`text-xs tracking-wide ${isActive ? 'text-[#1a2521]' : 'text-[#51615a] group-hover:text-[#1a2521]'}`}>
                  {tab.label}
                </span>

                {/* Accent line on active bottom */}
                {isActive && (
                  <div 
